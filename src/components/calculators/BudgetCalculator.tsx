...
const BUCKETS = [
  { key: 'needs', label: 'Needs', color: '#1f7a45', hint: 'Housing, utilities, groceries, insurance, minimum debt payments' },
  { key: 'wants', label: 'Wants', color: '#0ea5e9', hint: 'Dining out, entertainment, travel, subscriptions' },
  { key: 'savings', label: 'Savings & debt payoff', color: '#f59e0b', hint: 'Emergency fund, retirement, extra debt payments' },
] as const;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      ...
      <div className="mt-6 rounded-lg bg-brand-50 p-5" aria-live="polite">
        <p className="text-sm text-slate-600">Monthly take-home pay</p>
        <p className="text-3xl font-bold text-brand-800">{formatCurrency(r.monthlyTakeHome)}</p>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {BUCKETS.map((b) => (
            <div key={b.key} className="rounded-md bg-white p-4">
              <p className="flex items-center gap-2 font-semibold text-slate-900"><span className="h-3 w-3 rounded-sm" style={{ background: b.color }} />{b.label} ({pct[b.key]}%)</p>
              <p className="mt-1 text-2xl font-bold tabular-nums">{formatCurrency(r[b.key].monthly)}<span className="text-sm font-normal text-slate-500">/mo</span></p>
              <p className="text-sm text-slate-600">{formatCurrency(r[b.key].annual, { whole: true })} per year</p>
