...
function ClockFace({ hours24, minutes }: { hours24: number; minutes: number }) {
  ...
  const hand = (angle: number, length: number, width: number) => (
    <line x1="100" y1="100" x2={100 + length * Math.sin((angle * Math.PI) / 180)} y2={100 - length * Math.cos((angle * Math.PI) / 180)} stroke="#124b2e" strokeWidth={width} strokeLinecap="round" />
  );
  return (
    <svg viewBox="0 0 200 200" className="mx-auto h-48 w-48" role="img" aria-label="Life clock face">
      <circle cx="100" cy="100" r="92" fill="#fff" stroke="#1f7a45" strokeWidth="4" />
      {Array.from({ length: 12 }, (_, i) => {
        const a = (i * 30 * Math.PI) / 180;
        return <line key={i} x1={100 + 78 * Math.sin(a)} y1={100 - 78 * Math.cos(a)} x2={100 + 86 * Math.sin(a)} y2={100 - 86 * Math.cos(a)} stroke="#64748b" strokeWidth="3" />;
      })}
      {hand(hourAngle, 48, 6)}
      {hand(minuteAngle, 70, 3)}
      <circle cx="100" cy="100" r="5" fill="#124b2e" />
    </svg>
  );
}
...
export default function LifeClockCalculator() {
  ...
  return (
    <div className="grid gap-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 md:p-6">
      ...
      <div className="rounded-lg bg-brand-50 p-5 text-center" aria-live="polite">
        {result ? (
          <>
            <ClockFace hours24={result.clock.hours24} minutes={result.clock.minutes} />
            <p className="mt-3 text-sm text-slate-600">Your life clock reads</p>
            <p className="text-4xl font-bold text-brand-800">{result.clock.label}</p>
          ...
          <p className="text-slate-600">{birth ? 'Enter a birthdate in the past.' : 'Enter your birthdate to see your life clock.'}</p>
        )}
      </div>
    </div>
  );
