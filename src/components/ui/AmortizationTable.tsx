...
export function AmortizationTable({ rows, defaultOpen = false }: { rows: AmortizationRow[]; defaultOpen?: boolean }) {
  ...
  const th = 'px-3 py-2 text-right font-semibold text-gray-700';
  const td = 'px-3 py-1.5 text-right tabular-nums';
  return (
    <details className="mt-6 rounded-lg border border-gray-200 bg-white" open={defaultOpen}>
      <summary className="cursor-pointer px-5 py-3 font-semibold text-gray-900">Amortization schedule</summary>
      <div className="px-5 pb-5">
        <div className="mb-3 flex gap-2 text-sm">
          {(['yearly', 'monthly'] as const).map((v) => (
            <button key={v} type="button" onClick={() => setView(v)} className={`rounded-md px-3 py-1 ${view === v ? 'bg-brand-600 text-white' : 'border border-gray-300 text-gray-700'}`}>
              {v === 'yearly' ? 'By year' : 'By month'}
            </button>
          ))}
        </div>
        <div className="max-h-[28rem] overflow-auto" tabIndex={0} role="region" aria-label="Amortization schedule table">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-gray-50">
              <tr>
                <th className="px-3 py-2 text-left font-semibold text-gray-700">{view === 'yearly' ? 'Year' : 'Month'}</th>
